...
<input type="range" id="mv-pos-slider" min="100" max="900" value="300" style="width: 80px; height: 4px; border-radius: 2px; -webkit-appearance: none; background: rgba(255,255,255,0.2); outline: none; cursor: pointer;">            </div>
        ...
        setTimeout(() => {
            ...
            if (ambSlider) {
                ...
                ambSlider.onmousedown = () => { if (rootContainer) rootContainer.classList.remove('hide-cursor'); };
            }
        }, 100);
...
async function clickTranscriptLanguageOption(langName) {
    ...
    for (const el of buttons) {
        ...
        if (el.offsetParent !== null) { openDropdown = el; break; }
    }
    if (openDropdown) {
        openDropdown.click();
        ...
        const checkInterval = setInterval(() => {
            ...
            if (segmentNodes.length > 0) {
                ...
                const rawScraped = Array.from(segmentNodes).map(node => {
                    ...
                    return { time: timeSec, text: rawText };
                ...
                for (let i = 0; i < rawScraped.length; i++) {
                    ...
                    if (lines.length <= 1) {
                        ...
                        finalData.push({ time: current.time, text: current.text, el: null });
                    } else {
                        ...
                        lines.forEach((line, index) => {
                            ...
                            finalData.push({ time: interpolatedTime, text: line, el: null });
                        });
                    }
                }
...
async function fetchCaptionTrack(baseUrl) {
    try {
        ...
        let urlObj;
        try { urlObj = new URL(baseUrl); } catch (_) { }
...
if (text1 && text1.trim()) {
        ...
        if (baseUrl.includes('fmt=')) {
            urlJson = baseUrl.replace(/fmt=[^&]*/, 'fmt=json3');
        } else {
            urlJson = baseUrl.includes('?') ? baseUrl + '&fmt=json3' : baseUrl + '?fmt=json3';
        }
    ...
    try {
        ...
        if (json.events && Array.isArray(json.events)) {
            for (const ev of json.events) {
                ...
                segments.push({ time, duration, text: lineText, el: null });
            }
        }
    } catch (_) { }
...
function parseTimedTextXml(xmlText) {
    ...
    try {
        ...
        textNodes.forEach(node => {
            ...
            if (t) segments.push({ time: start, duration: dur, text: t, el: null });
        });
    } catch (e) { }
    return segments;
}
...
function buildLyricsUIWithLanguageSwitch(lyricsArea, lyricsHeader) {
    ...
    updateLangBtn(true);
    langBtn.onclick = () => { currentMode = (currentMode + 1) % 3; updateLangBtn(); };
...
function updateMetadataUI() {
    ...
    if (infoArea) {
        ...
        if (ambientBg) {
            const highResThumb = getHighResThumbnail();

            if (highResThumb && ambientBg.src !== highResThumb) {
                ambientBg.src = highResThumb;
            }
...
function getCaptionTracklistFromPlayer() {
    return new Promise((resolve) => {
        ...
        const handler = (e) => {
            ...
            let data = {};
            try { data = typeof e.detail === 'string' ? JSON.parse(e.detail) : (e.detail || {}); } catch (_) { }
            resolve({ list: data.list || [], active: data.active || null });
        };

        window.addEventListener(eventId, handler, { once: true });
        (document.head || document.documentElement).appendChild(script);

        setTimeout(() => resolve({ list: [], active: null }), 3000);
    });
}
...
if (!tracks || tracks.length === 0) {
    ...
    const savedLangCode = localStorage.getItem('mv_lyrics_lang');
    let initialTrack = validLangs.find(t => t.name === savedLangCode || t.code === savedLangCode) || validLangs[0];
let rawLyrics = await fetchCaptionTrack(initialTrack.baseUrl);
if (!rawLyrics || rawLyrics.length === 0) {
rawLyrics = await interceptFromCCButton();

        if (!rawLyrics || rawLyrics.length === 0) {
rawLyrics = await scrapeTranscriptFromDOM();
        }
    }
    ...
    lyricsData = processRawLyricsToProcessed(rawLyrics);
    currentLyricsLang = initialTrack.code;
    localStorage.setItem('mv_lyrics_lang', initialTrack.name);
    ...
    if (lyricsHeader) {
        ...
        if (validLangs.length > 0) {
            ...
            validLangs.forEach(item => {
                ...
                if (item.code === initialTrack.code) opt.selected = true;
                langSelect.appendChild(opt);
            });

            langSelect.onchange = async () => {
                const targetCode = langSelect.value;
                const targetTrack = validLangs.find(t => t.code === targetCode);
                if (!targetTrack) return;

                showToast('言語を切り替え中...');
                const newRawLyrics = await fetchCaptionTrack(targetTrack.baseUrl);

                if (newRawLyrics && newRawLyrics.length > 0) {
                    lyricsData = processRawLyricsToProcessed(newRawLyrics);
                    currentLyricsLang = targetCode;
                    localStorage.setItem('mv_lyrics_lang', targetTrack.name);
                    renderLyricsDOM(lyricsArea);
                    showToast(targetTrack.name + ' に切り替えました');
                } else {
    ...
    if (infoArea) {
        ...
        idleTimer = setTimeout(() => {
            ...
            const elems = [
                ...
                document.getElementById('mv-pos-slider-container'),
...
function syncLyrics() {
    ...
    if (activeIndex !== -1) {
        ...
        if (currentLine.isInstrumental && currentLine.el) {
            ...
            let progress = (elapsed / duration) * 3.1;
            progress = Math.max(0.0, Math.min(3.0, progress));

            currentLine.el.style.setProperty('--dot-progress', progress);

            const dots = currentLine.el.querySelectorAll('.instrumental-dot');
            dots.forEach(d => d.style.setProperty('--dot-progress', progress));
        }
        ...
        targetFrames.forEach((f, index) => {
            ...
            img.onclick = async (e) => {
                ...
                Object.assign(clone.style, {
                    ...
                    boxShadow: '0 0 0 rgba(0,0,0,0)', opacity: '1', animation: 'none'
                });
...
function showChekiEditor(prevFrames = null) {
    ...
    document.getElementById('cheki-cancel-btn').onclick = (e) => { e.stopPropagation(); overlay.remove(); };
