(function() {
        ...
        XMLHttpRequest.prototype.send = function() {
            this.addEventListener('load', function() {
                if (this._url && typeof this._url === 'string' && this._url.includes('/api/timedtext')) {
                    window.dispatchEvent(new CustomEvent('YTI_Intercepted_Captions', { detail: this.responseText }));
                }
            });
        ...
        window.fetch = async function(...args) {
            ...
            if (typeof url === 'string' && url.includes('/api/timedtext')) {
                response.clone().text().then(text => {
                    window.dispatchEvent(new CustomEvent('YTI_Intercepted_Captions', { detail: text }));
                }).catch(() => {});
            }
            return response;
        };
    ...
    try {
        ...
        if (typeof ytInitialPlayerResponse !== 'undefined' && ytInitialPlayerResponse?.captions?.playerCaptionsTracklistRenderer?.captionTracks) {
            tracks = ytInitialPlayerResponse.captions.playerCaptionsTracklistRenderer.captionTracks;
        }

        const player = document.getElementById('movie_player');
        if (tracks.length === 0 && player && typeof player.getPlayerResponse === 'function') {
    ...
    window.dispatchEvent(new CustomEvent(eventId, { detail: JSON.stringify({ list: tracks }) }));
})();
